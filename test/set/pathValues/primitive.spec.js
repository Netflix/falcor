...
describe("a primitive value", function() {
    it("throws with a `null` key in a branch position", function() {
        ...
        try {
            setPathValues(
                ...
                [
                    $pathValue(
                        ["movies", null, "pulp-fiction", "title"],
                        "Pulp Fiction"
                    )
                ]
            );
    ...
    it("directly", function() {
        ...
        expect(strip(cache)).to.deep.equal(
            strip({
                movies: {
                    "pulp-fiction": {
    ...
    it(
        "through a reference that lands on an atom with a null last key",
        function() {
            var cache = {};
            var version = 0;
            setPathValues(getModel({ cache: cache, version: version++ }), [
    ...
    describe("in multiple places", function() {
        describe("via keyset", function() {
            ...
            it(
                "through successful, short-circuit, and broken references",
                function() {
                    var cache = {};
                    var version = 0;
                    setPathValues(
                        ...
                        [
                            ...
                            $pathValue(
                                "lists['id'][0]",
                                $ref("movies['pulp-fiction']")
                            ),
                            $pathValue(
                                "lists['id'][1]",
                                $ref("movies['kill-bill-1']")
                            ),
                            $pathValue(
                                "lists['id'][2]",
                                $ref("movies['reservior-dogs']")
                            ),
                            $pathValue(
                                "movies['kill-bill-1'].director",
                                $atom()
                            ),
                            $pathValue(
        ...
        describe("via range", function() {
            ...
            it("from:1, length:2", function() {
                ...
                setPathValues(getModel({ cache: cache, version: version++ }), [
                    ...
                    $pathValue("movies['kill-bill-1'].director", $atom()),
                    $pathValue(
                        ["grid", 0, { from: 1, length: 2 }, "director"],
                        "Quentin Tarantino"
                    )
                ]);
