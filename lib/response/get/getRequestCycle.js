...
module.exports = function getRequestCycle(getResponse, model, results, observer,
    ...
    getResponse.responses[getResponse.responses.length - 1].concurrentRequestCount =
      requestQueue._requests.filter(function (req) { return req.sent; }).length
    ...
    var currentRequestDisposable = requestQueue.
        get(boundRequestedMissingPaths, optimizedMissingPaths, function(err, data, hasInvalidatedResult) {
            ...
            if (hasInvalidatedResult) {
                ...
                nextSeed = [{}];
                getResponse.responses[getResponse.responses.length - 1].didInvalidateResults = true;
            } else {
                nextRequestedMissingPaths = requestedMissingPaths;
                nextSeed = results.values;
            }

            getResponse.responses[getResponse.responses.length - 1].response = data;
            ...
            if (nextResults) {
                ...
                disposable.currentDisposable =
                    getRequestCycle(getResponse, model, nextResults, observer,
                                    errors, count + 1);
                getResponse.responses.push({
                    requestedMissingPaths: nextResults.requestedMissingPaths.concat(),
                    optimizedMissingPaths: nextResults.optimizedMissingPaths.concat()
                });
            }
