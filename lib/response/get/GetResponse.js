...
GetResponse.prototype._subscribe = function _subscribe(observer) {
    ...
    this.responses.push({
        requestedMissingPaths: results.requestedMissingPaths.concat(),
        optimizedMissingPaths: results.optimizedMissingPaths.concat(),
        concurrentRequestCount: 0, // gets reset in getRequestCycle immediately
        didInvalidateResults: false, // gets potentially reset after response
        selectiveCache: serializeCache(model, ['lolomos', 'lolomo', 'lists']),
    });
...
function serializeCache(model, paths) {
  return paths.reduce(function (cache, path) {
    cache[path] = JSON.stringify(model._root.cache[path], ignoreInternal);
    return cache;
  }, {});
}

function ignoreInternal(key, value) {
  return key.indexOf('$') === 0 ? undefined : value;
}
