        ...
        if(results && results.invokeSourceRequest === true) {
            ...
            return (model._request
                ...
                .do(function setResponseEnvelope(envelope) {
                        ...
                        envelope.paths = optimizedPaths;
                        model._path = empty_array;
                        set_json_graph_as_json_dense(model, [envelope], empty_array, source.errorSelector, source.comparator);
                        model._path = boundPath;
                    },
                    function setResponseError(error) {
                        source.isCompleted = true;
                        model._path = empty_array;
                        set_json_values_as_json_dense(model, array_map(results.optimizedPaths, function(path) {
...
function subscribeToSetResponse(observer) {
    ...
    var comparator = this.subscribeCount++ > 0 && this.comparator || undefined;
