...
module.exports = function getCachePosition(model, path) {
    ...
    return [
        // the cache position from the path resolution.
        currentCachePosition,

        // The short happens when there is a cache position from the path and
        // the depth is less than the maxDepth
        depth !== maxDepth && currentCachePosition
    ];
};
