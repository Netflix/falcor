...
var RequestQueue = require("falcor/requests/RequestQueue");
var GetResponse = require("falcor/responses/GetResponse");
var SetResponse = require("falcor/responses/SetResponse");
var CallResponse = require("falcor/responses/CallResponse");
var ModelResponse = require("falcor/responses/ModelResponse");
...
function Model(options) {
    ...
    this._root = options.root || options._root || new ModelRoot();
    ...
    if(options.treatErrorsAsValues || options.hasOwnProperty("_treatErrorsAsValues")) {
        this._treatErrorsAsValues = options.treatErrorsAsValues || options._treatErrorsAsValues;
    }

    if(options.comparator || options.hasOwnProperty("_comparator")) {
        this._comparator = options.comparator || options._comparator;
    }
...
Model.prototype._errorSelector = function _errorSelector(x, y) { return y; };
Model.prototype._comparator = function _comparator(a, b) {
...
Model.prototype.invalidate = require("falcor/invalidate");
...
Model.prototype.setCache = function setCache(cacheOrJSONGraphEnvelope) {
    ...
    if(cacheOrJSONGraphEnvelope !== cache) {
        ...
        if(typeof cache !== "undefined") {
            collect_lru(this._root, this._root.expired, get_size(cache), 0);
        }
        if(is_json_graph_envelope(cacheOrJSONGraphEnvelope)) {
            ...
            set_cache(this, { json: cacheOrJSONGraphEnvelope });
        }
...
Model.prototype.syncCheck = function syncCheck(name) {
    if (Boolean(this._source) && this._root.allowSync <= 0 && this._root.unsafeMode === false) {
...
Model.prototype.clone = function clone() {
    ...
    var argsLen = arguments.length;
    while(++argsIdx < argsLen) {
        var keyValue = arguments[argsIdx];
        var key = keyValue[0]
        var value = keyValue[1];
        if(value === false && clone.hasOwnProperty(key)) {
            delete clone[key];
        } else {
            clone[key] = value;
        }
...
Model.prototype.dematerialize = function materialize() {
    return this.clone(["_materialized", false]);
};
...
Model.prototype.withoutDataSource = function withoutDataSource() {
    return this.clone(["_source", false]);
};

Model.prototype.withComparator = function withComparator(compare) {
    return this.clone(["_comparator", compare]);
};

Model.prototype.withoutComparator = function withoutComparator() {
    return this.clone(["_comparator", false]);
};
...
Model.prototype._setPathSetsAsJSON = require('falcor/set/set-json-values-as-json-dense');
Model.prototype._setPathSetsAsJSONG = require('falcor/set/set-json-values-as-json-graph');
Model.prototype._setPathSetsAsPathMap = require('falcor/set/set-json-values-as-json-sparse');
Model.prototype._setPathSetsAsValues = require('falcor/set/set-json-values-as-json-values');
...
Model.prototype._invPathSetsAsJSON = require('falcor/invalidate/invalidate-path-sets-as-json-dense');
Model.prototype._invPathSetsAsJSONG = require('falcor/invalidate/invalidate-path-sets-as-json-graph');
Model.prototype._invPathSetsAsPathMap = require('falcor/invalidate/invalidate-path-sets-as-json-sparse');
Model.prototype._invPathSetsAsValues = require('falcor/invalidate/invalidate-path-sets-as-json-values');
