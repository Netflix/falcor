...
var hasIntersection = pathUtils.hasIntersection;
...
GetRequestV2.prototype = {
    ...
    add: function(requested, optimized, callback) {
        ...
        var self = this;
        var outPaths = complement(requested, optimized, self._pathMap);
        var optimizedComplement = outPaths[1];
        var inserted = false;
        ...
        if (outPaths.length < paths.length) {
            ...
            self._requestedPaths[idx] = outPaths[2];
    ...
    _merge: function(requested, err, data) {
        ...
        if (err) {
            ...
            var pathValues = requested.map(function(x) {
                return {
                    path: path,
                    value: error
                };
...
// Creates the complement of requested / optimized paths
function complement(requested, optimized, tree) {
    var out = [];
    var outRequested = [];
