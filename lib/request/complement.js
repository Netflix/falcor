...
module.exports = function complement(requested, optimized, tree) {
    ...
    for (var i = 0, len = optimized.length; i < len; ++i) {
        ...
        // fully deduped
        else if (hasIntersection(subTree, oPath, 0)) {
        ...
        // partial deduping
            ...
            if (depthDiff === 0) {
                intersectionData = findPartialIntersections( rPath, oPath, subTree, 0, [], [], 0 );
            } else if (depthDiff > 0) {
                intersectionData = findPartialIntersections( rPath, oPath, subTree, 0, arraySlice(rPath, 0, depthDiff), [], depthDiff );
            } else {
                // depthDiff < 0
                // Pre-descend into the sub tree
                for (var l = 0; subTree && l < -depthDiff; l++) {
                    subTree = subTree[oPath[l]];
                }

                intersectionData = findPartialIntersections( rPath, oPath, subTree, -depthDiff, [], arraySlice(oPath, 0, -depthDiff), depthDiff );
            }
 ...
 *      - depthDiff: 1
 */
function findPartialIntersections(
    requestedPath,
    optimizedPath,
    currentTree,
    depth,
