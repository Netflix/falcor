...
RequestQueueV2.prototype = {
    ...
    get: function(requestedPaths, optimizedPaths, cb) {
        ...
        var i, len;
        var remainingPaths = optimizedPaths;
        var disposed = false;
        var request;
        ...
        for (i = 0, len = requests.length; i < len; ++i) {
            ...
            // if possible.
            if (request.sent) {
                var results = request.add(remainingPaths, refCountCallback);

                // Checks to see if the results were successfully inserted
                // into the outgoing results.  Then our paths will be reduced
                // to the complement.
                if (results[0]) {
                    remainingPaths = results[1];
                    disposables[disposables.length] = results[2];
                    ++count;
                }
            }
            ...
            // If there is a non sent request, then we can batch and leave.
                ...
                request.batch(remainingPaths, refCountCallback);
                remainingPaths = [];
                ++count;
            }
        ...
        if (remainingPaths.length) {
            ...
            ++count;
            var disposable = request.batch(optimizedPaths, refCountCallback);
