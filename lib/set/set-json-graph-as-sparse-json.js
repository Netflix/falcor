module.exports = set_json_graph_as_sparse_json;
...
function set_json_graph_as_sparse_json(model, envelopes, values, error_selector) {
...
function onNode(pathset, roots, parents, nodes, requested, optimized, is_top_level, key, keyset, is_keyset) {
    ...
    if(key == null) {
        ...
        jsonkey = get_valid_key(null, requested);
        json   = parents[3];
        parent = parents[0];
    ...
    if(pathset.length > 1) {
        parents[0] = node;
        parents[2] = message;
        if(!!(parents[3] = json) && (!type || type == $path)) {
