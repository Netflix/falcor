/*
TODO: REVIEW NOTES FROM SATYEN

# GENERAL

1. "HOME" needs to be Falcor.
2. "HOME" page is empty in generated docs. Could benefit from a boxes/arrows high-level class relationship overview/image.
3. Examples should be snippets, and we should have complete end-to-end examples with walkthroughs (linked from API Docs).
4. We should have an index of methods/properties for each class with args/return [ which links to the detailed docs ].
5. Would be nice to see hierachy info alongside class (as opposed to after Example/Description/Params). e.g. HttpDataSource
   extends DataSource
6. Break up descriptions into paragraphs
7. new Model(optionsnullable) - the nullable is a little messy. Can't we just use the standard new Model([some optional arg])
8. Parameters tables - Would it be more easily scannable if combine type/attributes? [String], ...String, etc..

# DataSource

1. `(abstract) new DataSource()` doesn't make sense
2. Description has "marketing" blurb - e.g. "A Falcor Model allows you to access the data in a JSON ..."
3. Source links don't work (will when we move docs to real codebase)
4. `get` Member doc is missing args/return value (first thing you see in DataSource)
5. Observable.<JSONGraphEnvelope> should be Observable<JSONGraphEnvelope>

# Http DataSource

1. Unneccesary verbage - "The DataSource interface includes a set method which can be called to modify a remote model.". We're
   already looking at the "DataSource" class [ we have that context already ].

2. Typo - "returns an Observable. which contains"

# Model

1. Needs a modes/get/set behavior matrix at the top of the page, which can be referenced from the get/set docs.
2. Default values of all modes should be aggregated in the Model constructor docs [ that is, if I new up a new Model, what modes is it in by default]
2. Would be good to organize methods by frequency of use (that is, have get/set/bind/call up top, as opposed to batch).
3. Method/Property/Class references should be cross-linked (e.g. getValueSync, when it refers to get)
4. Not sure if this applies anymore, but mode methods (batch(), boxValues() etc.) should clarify
   whether they are persistent or transient. Came up when we were discussing potentially adding toJSON(), toJSONG() as modes.
5. Verbage cleanup - "The getValue method retrieves a single Path from a Model.", should be "retrieves the value at a single Path",
   or something similar (value, atom, sentinel ??).
6. Caveats/Edge Cases should be "bolded". e.g. "This API is inherently unsafe and should only be used if you know what you're doing"

# Global

1. A bunch of, what should probably be falcor.* classes seem to have ended up in Global [ Atom, Key, etc.]
2. Atom seems to be missing Example format separator [ticks]

# Observable

1. http://netflix.github.io/falcor/doc/Observable.html#~onNextCallback - Is this really a type?
...
/**
 * Invokes a function in the JSON Graph object exposed by the {@link DataSource}.
 * @name call
 * @function
 ...
 * @arg {Array.<PathSet>} calleePaths paths to retrieve from function callee after successful Function execution
 * @returns {Observable.<JSONGraphEnvelope>} jsonGraphEnvelope the response returned from the server.
 * @memberof DataSource.prototype
 * @example
var model = new falcor.Model({source: new falcor.HttpDataSource("http://netflix.com/model.json")});
var $ref = falcor.Model.ref;
...
/**
 * A HttpDataSource object is a {@link DataSource} can be used to retrieve data from a remote JSONGraph model using the browser's {@link HttpDataSource} object.
...
/**
 * Returns a copy of the {@link Model} that attempts to batch concurrent requests of the same type. If two requests for different paths are made to a batched {@link Model} within a time slice defined by a {@link Scheduler} or a number of milliseconds, a single request for both paths will be made to the {@link Model}'s {@link DataSource}. If no scheduler or time is specified, the batch will be formed as soon as possible. Enabling batching on a {@link Model} can make more efficient use of the network by reducing the number of requests to the server.
 * @name batch
 * @memberof Model.prototype
 * @function
 ...
 * @see Atom
 * @function
 * @return {Model} a {@link Model} that the values inside of {@link Atom}s rather than the sentinel instances themselves.
 */
...
//TODO: is call() still not implemented?
