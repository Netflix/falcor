...
function set_json_graphs_as_json(model, envelopes, values) {
    ...
    while(++index < count) {
        ...
        while(++index3 < count2) {
            ...
            var json    = values && values[++index2];
            if(json != null) { json = json.json || (json.json = {}); }
            ...
            if(hasValue && values && values[index2]) {
                values[index2].json = json;
                delete roots.json;
            }
        }
    }
...
function on_link(roots, parents, nodes, key) {
    ...
    var node    = nodes[0] = parent[key];
    var message = nodes[2] = messageParent && messageParent[key];
    nodes[0] = merge_node(roots, parent, node, messageParent, message, key);
}
