/**
 * A DataSource object is used by the {@link Model} to retrieve data from a remote JSONG model. The {@link Model} and its {@link DataSource} work together to optimize and hide network access from developers. The {@link Model} tries to retrieve paths from its local cache initially, and uses the {@link DataSource} to retrieve missing information from the server. {@link DataSource} is an interface that should be implemented for a given transport layer.
 * @constructor DataSource
 * @abstract
 * @example
 // A {@link Model} and its {@link DataSource} work together to optimize and hide network access from developers. In this example we will create a mock implementation of a DataSource to demonstrate how the {@link Model} and {@link DataSource} interact.
 ...
 // server.
 var mockDataSource = {
    // expecting one request for ["titles", 99, ["name", "rating"]]
    get: function() {
        ...
        // return an Observable stream of the {@link JSONGEnvelope} that a server would
 // send in response to a request for ["titles", [99, 23, 44], ["name", "rating"]]
 return Observable.of({
            paths: [["titles", 99, ["name", "rating"]]],
            value: {
                "titles": {
                    "99": {
                        "name": "House of Cards",
 ...
 var model = new jsong.Model(mockDataSource);

 // Loading a fragment of the JSONG object into the model's cache.
 model.setCache(
 {
 ...
 var namesAndRatings = model.get(["genreList", 0, {to:1}, ["name","rating"]]);
 ...
 model.
 deserialize().
 subscribe(function(jsong) {
            console.log("Contents of {@link Model}'s local cache after get opration:")
 console.log(JSON.stringify(jsong));
 });
 });

 // Attempting to retrieve the {@link PathSet} ["genreList", 0, {to:1}, ["name", "rating"]]
 // from the Model will cause the paths to be first evaluated against the local
 // cache. Based on {@link Path}s found in the local JSONG cache, the requested {@link PathSet}s
 // will be optimized into the following {@link PathSets}:

 // ["genreList", 0, 0, ["name", "rating"]] -> ["titles", 23, ["name", "rating"]]
 // ["genreList", 0, 1, ["name", "rating"]] -> ["titles", 99, ["name", "rating"]]

 // Of these optimized {@link PathSet}s, only the following {@link PathSet} cannot
 // be found in the cache: ["titles", 99, ["name", "rating"]]. A request is
 ...
 // JSONG fragment, it is sent to the {@link Model}. The {@link Model} adds the
 ...
 // Paths requested from DataSource: ["titles", 99, ["name", "rating"]]
...
//     }
// }
 // Contents of {@link Model}'s local cache after get opration:
 // {
//     "genreList": {
//         "0": ["genres", 123],
//         "length": 1
...
/**
 * The get method retrieves {@link Path}s or {@link PathSet}s from a remote JSONG model.
 * @name get
 * @arg {...PathSet} path the path(s) to retrieve
 * @returns {Observable.<JSONGEnvelope>} jsongEnvelope the response returned from the server.
...
/**
 * @name set
 * @arg {...PathValue} pathValue the series of path and value pairs to set on the remote JSONG model.
 * @returns {Observable.<JSONGEnvelope>} jsongEnvelope the response returned from the server.
 * @memberof DataSource.prototype
 */
 ...
 * @example
 var model = new jsong.Model(new jsong.XMLHttpDataSource("http://netflix.com/user.jsong"));
 var movieNames = model.getValues(["lolomo",{length:10},{length:10}, "name"]);
 */

/**
 * A JSONGEnvelope is an envelope that wraps a JSONG fragment. In addition to {@link PathValue}s and {@link JSONEnvelope}s, {@link Model}s can accept and return JSONGEnvelopes. In addition to a JSONG fragment, a {@link JSONGEnvelope} may contain errors that occurred while an operation was being executed on a JSONG Model. In the event a function was executed on a Model, the JSONGEnvelope may also contain a list of paths to invalidate in the {@link Model}'s local cache.
 * @typedef {Object} JSONGEnvelope
 * @property {?Array.<PathSet>} paths - The paths to the values stored in the JSONG fragment.
 * @property {JSONG} jsong - a JSONG object containing all of the values.
 * @property {CompositeError | Error} error - one or more errors that occurred while evaluating paths on a {@link Model}.
 * @property {?Array.<PathSet>} invalidated - the paths to be removed from the {@link Model}'s local cache.
 */

/**
 * A JSONEnvelope is an envelope that wraps a JSON fragment. In addition to {@link PathValue}s and {@link JSONGEnvelope}s, {@link Model}s can accept and return JSONEnvelopes. In addition to a JSON fragment, a {@link JSONEnvelope} may contain errors that occurred while an operation was being executed on a JSON Model. In the event a function was executed on a Model, the JSONEnvelope may also contain a list of paths to invalidate in the {@link Model}'s local cache.
...
/**
 * JSONG values can include metadata that dictates how the Model handles them. For example metadata can be used to control how long a value is stored in a Model's local cache. However metadata attached to JSONG values like as strings, numbers, and Arrays will not survive the JSON serialization process. A Sentinel object one several types of special JSONG objects known as Atoms. Atoms are maps with a "$type" property. Although Atoms are maps, they behave like values and are always returned whole. Sentinel atoms are maps with a $type value of "sentinel," and they can be used wrap a JSONG value so that metadata can be attached to it. This is known as "boxing." A Sentinel can box any JSONG value except an Atom.
 * By default {@link Model}s do not return Sentinels. An attempt to retrieve a Sentinel from a {@link Model} will yield its boxed value rather than the Sentinel itself. It is possible to change this default by calling {@link Model.prototype.boxValues}. This method can be used in order to retrieve the metadata associated with value, which is stored on its Sentinel object.
 * @typedef {Object} Sentinel
 * @property {String} $type - A $type property with a value of "sentinel."
 * @property {Object} value - The value wrapped by the Sentinel.
 * @see {@link Sentinel}
 * @example
 var model = new jsong.Model(new jsong.XMLHttpDataSource("http://netflix.com/user.jsong"));
 model.setCache({
 ...
 // Jim Parsons
 // Note that the value property within the Sentinel rather than the Sentinel object itself is returned.
 */
 ...
 * @example
 ...
 var results = model.getValues(["friends", range, "name"]);
...
/**
 * {@link Key}s are values that can be included in {@link Path} expressions. A Key can be any JSON value type including a string, number, boolean, or null. When evaluated all keys are coerced to strings except null. This means that looking up the number 1 and the string "1" will have the same effect. All strings are allowed with the exception of those beginning with "__", which are illegal to use as key names in a JSONG model.
 * @typedef {(string|number|boolean|null)} Key
 */

/**
 * {@link Path}s are an ordered list of keys describing a location in a JSONG model. Paths can be provided to {@link Model}s to describe the location of data in the JSONG model. {@link Model}s also emit {@link Path} objects alongside values retrieved from the JSONG model in order to provide additional context.
 * @typedef {Array.<Key>} Path
 @example
 var model = new Model();

 // Loading a JSONG object into the model's cache.
 model.setCache(
 {
     // list of user's genres, modeled as a map with ordinal keys
     "genreList": {
 ...
 // Attempt to retrieve three individual paths from the JSONG model
 var values = model.
 get(
 ["genreLists", 0, 0, "name"],
 ...
 // The code above outputs the following {@link PathValue}s to the console in non-deterministic order.
 // {"path":["genreList",0,1,"name"],"value":"Orange is the New Black"}
 // {"path":["genreList",0,0,"rating"],"value":5}
 // {"path":["genreList",0,0,"name"],"value":"House of Cards"}
 */

/**
 * {@link KeySet}s are included inside of {@link PathSet} expressions. A complex key can either be a {@link Key}, a {@link Range}, or an Array of either.
 ...
 * {@link PathSet} Expressions are a human-readable compression format that is equivalent to a set of {@link Path} expressions. Unlike {@link Path} expressions, {@link PathSet}s can contain {@link KeySet} objects like {@link Range}s and Arrays of keys.
 In addition to being a more terse tha specifying multiple {@link Path} objects, {@link PathSet}s are generally more efficient to evaluate.
 * @typedef {Array.<KeySet>} PathSet
 * @example
 var model = new jsong.Model();
...
/**
 * A PathValue is a small envelope around a value retrieved from a JSONG model. In addition to the value, each {@link PathValue} includes the {@link Path} to the value in the JSONG model.
 ...
 * @example
 ...
 // The code above outputs the following {@link PathValue}s to the console in non-deterministic order.
 // {"path":["genreList",0,0,"rating"],"value":5}
 // {"path":["genreList",0,0,"name"],"value":"Orange is the New Black"}
...
/**
 * A {@link CompositeError} object stores one or more errors encountered concurrently during a attempt to perform several concurrent operations on a {@link Model}.
 * @typedef CompositeError
 * @property {Array.<Error>} innerErrors An array of errors encountered while attempting to perform several concurrent operations on a {@link Model}.
 * @example
 var model = new jsong.Model(new jsong.XMLHttpDataSource("http://urlthatdoesntexist.com"));

 var results =
 model.
 get(["pathThatResultsInError"], ["anotherPathThatResultsInError"]).
 toPathValues();

 results.forEach(
 function onNext(pathValue) {
        // this method will not be invoked because none of the path values were able to be retrieved.
    },
 function onError(compositeError) {
        JSON.stringify(compositeError);
    });

 // The code above prints the following output to the console:
 // {
//     "innerErrors":[
//         {path:["pathThatResultsInError"],message:"404"},
//         {path:["anotherPathThatResultsInError"],message:"404"}
//     ]
// }
 */

/**
 * JavaScript Object Notation Graph (JSONG) is a notation for expressing graphs in JSON. JSON is a hierarchical data format, capable of modeling trees. Here's an example of a JSON document containing a list of genres, each containing a list of titles:
 * @example
 // JSONG model modeling a list of film genres, each of which contains the same title.
 {
    // list of user's genres, modeled as a map with ordinal keys
    "genreList": {
        "0": ["genres", 123],
 ...
 * @example
 ...
 /**
 * Converts the data format of the data in a JSONG Model response to a stream of path values.
 * @name toPathValues
 ...
 * @example
 ...
 var response = model.get(["genreList", 0, 0, ["name","rating"]]);
...
/**
 * Converts the data format of the data returned in a Model response to JSONG.
 * @name toJSONG
 * @memberof ModelResponse.prototype
 * @function
 * @return ModelResponse.<JSONGEnvelope>
 * @example
 var model = new jsong.Model();
 ...
 response.toJSONG().forEach(function(jsongEnvelope) {
    console.log(JSON.stringify(jsongEnvelope));
});

 // The code above outputs the following {@link JSONGEnvelope} to the console in non-deterministic order.
 // {
//     "jsong": {
//         // list of user's genres, modeled as a map with ordinal keys
//         "genreList": {
 ...
 * @example
 ...
 // The code above outputs the following {@link JSONEnvelope} to the console in non-deterministic order.
 // {
//     "json": {
//         // list of user's genres, modeled as a map with ordinal keys
//         "genreList": {
 ...
 * @example
 ...
 nameAndRatings.subscribe(function(value) {
    ...
    var html = "",
        json = value.json;
    if (data.genreList && json.genreList[0]){
        var genre = json.genreList[0];
        html += "&lt;h1&gt;" + genre + "&lt;/h1&gt;\n" +
        "&lt;ul&gt;\n;
 ...
 // &lt;/ul&gt;
 * @example
 // In this example, we will perform the same operation as the last example: we will retrieve data from a local JSONG model cache and convert it into an HTML list. In the previous example, we converted the JSONG in the cache to JSON and then subsequently into HTML. In this example, we will avoid this intermediary JSON transformation by providing a selector function to the get method. Within the selector function, we will access the JSONG data in the Model's local cache and transform it directly into HTML.

 // Creating a model and passing a data source that connects it to a remote JSONG model.
 var model = new jsong.Model(new XMLHttpDataSource('http://netflix.com/model.jsong'));

 // Loading a JSONG object into the model's cache. This JSONG object models a list of genres, each of which contains several titles.
 model.setCache(
 {
     // list of user's genres, modeled as a map with ordinal keys
     "genreList": {
 ...
 var html = model.get(
 ["genreList", 0, {to:1}, ["name","rating"]], ["genreList", 0, "name"],
 ...
 function() {
        var firstGenreList = model.bindSync(["genreList",0]);
 ...
 // &lt;/ul&gt;
 * @example
 // In this example, we will perform the same operation as the last example: we will retrieve data from a local JSONG model cache and convert it into an HTML list. In the previous example, we passed a selector function to the get method and transformed the JSONG data in the cache directly into HTML. This improved performance by removing the need to created an intermediary JSON object, but required us to repeat the path twice: once in the method's argument list, and again in the body of the selector function. In this example, we will achieve a good balance between performance and expressiveness by passing arguments to the selector function.
 ...
 // {
    ...
    //     }
    // }
 // Notice that of the path ["genreList",0,{to:1}, ["name","rating"]] the {@link Key}s ["genreList",0] in the path are not present in the structure, only the {@link KeySet}s [{to:1}, ["name","rating"]]. This optimization decreases allocations and repetitive boiler plate code.
 // genreName is bound to the value of ["genreList", 0, "name"], which is "Drama".
 ...
 * @example
 ...
 // Within the selector function we read data directly from the cache, and convert it directly into HTML.
 var html = model.get(
 ["genreList", {to:1}, {to:1}, "name"], ["genreList", {to:1}, "name"],
 // Arguments passed to the selector function will be bound to the values of the paths.
 // The process of binding path values to the arguments of selector functions works like this:
 // If a {@link PathSet} is bound to a selector function argument, all {@link Key}s are removed from the JSON data structure, leaving behind only the keys in the {@link KeySet}s.
 // names is bound to the value of ["genreList", {to:1}, {to:1}, "name"], which is:
 // {
    //     "0": {
    ...
    //     }
    // }
 // Notice that of the path ["genreList",{to:1},{to:1}, "name"] the {@link Key}s ["genreList"] and ["name"] in the path are not present in the structure, only the {@link KeySet}s [{to:1},{to:1}]. This optimization decreases allocations and repetitive boiler plate code.
 // genreNames is bound to the value of ["genreList", {to:1}, "name"], which is:
 // {
    //     "0": "Drama",
    //     "1": "Comedy"
    // }
 // Notice once again that the {@link Key}s ("genreList","name") have been removed from the structure, leaving only the keys in the {@link KeySet} ({to:1}) in the data structure.
 ...
 * @example
 ...
 var model = new jsong.Model();
 // Loading a JSONG object into the Model's cache to mock a server model locally.
 model.setCache(
 {
     user: {
         name: "Frank Underwood",
         hometown: {$type: 'error', message: 'Could not be retrieved from server.'},
         // A JSONG Error is any map that contains a $type key of "error." Errors are JSONG Atoms, which are special maps that act like values in that they are always returned whole. Errors are handled differently than normal values by the Models.
         country: {$type: 'error', message: 'Could not be retrieved from server.'}
     }
 });
 ...
 * @example
 ...
 // result in a request to the server for ["genreList", 0, 2, ["name","rating"]] because the third title
 // is not in the cache. The selector function passed to getProgressively will be called twice, first
 // with the data loaded from the cache, and the second time with the additional data retrieved from the server.
 // Each time the selector function is invoked, there is more data. The selector function converts the
 // data present into an HTML representation.
 var nameAndRatings = model.getProgressively(
 ["genreList", 0, {to:2}, ["name","rating"]], ["genreList", 0, "name"],
...
/**
 * The getProgressively method retrieves several {@link Path}s or {@link PathSet}s from the JSONG object, and makes them available in the local cache. Like the {@link Model.prototype.getProgressively} function, getProgressively invokes a selector function every time is available, creating a stream of objects where each new object is a more populated version of the one before. The getProgressively function is a memory-efficient alternative to the getProgressively function, because get does not convert the requested data from JSONG to JSON. Instead the getProgressively function attempts to ensure that the requested paths are locally available in the cache when it invokes a selector function. Within the selector function, data is synchronously retrieved from the local cache and translated into another form - usually a view object. Within the selector function you can use helper methods like getValueSync and setValueSync to synchronously retrieve data from the cache. These methods are only valid within the selector function, and will throw if executed anywhere else.
 * @name getProgressively
 * @memberof Model.prototype
 * @function
 * @arg {...PathSet} path the path(s) to retrieve
...
/**
 * The getProgressively method retrieves several {@link Path}s or {@link PathSet}s from the JSONG object, and returns the requested data as an {@link Observable} sequence of JSON tree. Returning the requested data as a JSON tree rather than a JSONG object makes it easy to send the data into a template. The method is called getProgressively because the JSON tree is progressively populated as data arrives. In this regard this method differs from the {@link Model.prototype.get} method, which waits until all the data is present before returning a result in the {@link Observable} stream. Each time a new {@link JSONGEnvelope} arrives, it is added to the tree, and the tree is added to the sequence again. This method is ideal if you want to render the data that is present in the local cache immediately and then rerender each time data arrives from the remote model on the server.
...
/**
 * Returns a copy of the {@link Model} that uses only the data in the local cache. The new {@link Model} never makes a request to a remote JSONG model using a {@link DataSource} if data cannot be found in the local cache.
 * @name invalidate
 * @memberof Model.prototype
 ...
 * @memberof Model.prototype
 * @function
 * @return {Model} a {@link Model} that uses only the data in the local cache, and never makes a request to a remote JSONG model if data cannot be found locally.
 */

/**
 * Returns a copy of the {@link Model} that attempts to batch concurrent requests of the same type. If two requests for different paths are made to a batched {@link Model} within a time slice defined by a {@link Scheduler} or a number of milliseconds, a single request for both paths will be made to remote JSONG model. If no scheduler or time is specified, the batch will be formed as soon as the current tick has finished. Enabling batching on a {@link Model} can make more efficient use of the network by reducing the number of requests to the server.
 * @name batch
 * @memberof Model.prototype
 * @function
 ...
 * @return {Model} a {@link Model} that batches requests of the same type and sends them to the data source together.
 @example
 // In this example we will demonstrate that multiple calls made to a batched JSONG model in a single tick will only result in a single request to the {@link DataSource} responsible for retrieving data from the server.
 ...
 // server.
 var mockDataSource = {
    // expecting one request for ["titles", [99, 23, 44], ["name", "rating"]]
    get: function(paths) {
        requestCount++;

        // return an Observable stream of the {@link JSONGEnvelope} that a server would return
 return Observable.of({
            paths: [["titles", [23,44, 99], ["name", "rating"]]],
            value: {
                "titles": {
                "99": {
                    "name": "House of Cards",
 ...
 * @return {Model} a {@link Model} that batches requests of the same type and sends them to the data source together.
 ...
 var mockDataSource = {
    // expecting two calls:
    // 1. ["titles", [99, 23, 44], "name"]
    // 2. ["titles", [99, 23, 44], "rating"]
    get: function(path) {
        requestCount++;

        // if path is ["titles", [99, 23, 44], "name"]
        if (path[2] === "name") {
            // return an Observable stream of the {@link JSONGEnvelope} that the server would send
 return Observable.of({
                paths: [["titles", [23,44, 99], "name"]],
                value: {
                    "titles": {
                    "99": {
                        "name": "House of Cards"
 ...
 // if path is ["titles", [99, 23, 44], "rating"]
 else {
            // return an Observable stream of the {@link JSONGEnvelope} that the server would send
 return Observable.of({
                paths: [["titles", [23,44, 99], "rating"]],
                value: {
                    "titles": {
                    "99": {
                        "rating": 5
 ...
 // creating an Observable stream of a JSON object that contains the names of the first two titles in the first genre list
 var names = unbatchedJsongModel.get(["genreList", 0, {to:1}, ["name"]]);

 // creating an Observable stream of a JSON object that contains the ratings of the first two titles in the first genre list
 var ratings = unbatchedJsongModel.get(["genreList", 0, {to:1}, ["rating"]]);
...
/**
 * Returns a copy of the {@link Model} that unboxes {@link Sentinel} values before returning them. By default {@link Model}s unbox values before returning them.
 * @name unboxValues
 * @memberof Model.prototype
 * @see Sentinel
 * @function
 * @return {Model} a {@link Model} that the values inside of {@link Sentinel}s rather than the sentinel instances themselves.
 */

/**
 * Returns a copy of the {@link Model} that does not unbox {@link Sentinel} values before returning them. Returning {@link Sentinel}s rather than their values allows any metadata attached to the value to be inspected.
 * @name boxValues
 * @memberof Model.prototype
 * @see Sentinel
 * @function
 * @return {Model} a {@link Model} that returns sentinel values rather than the value inside them.
 * @example
 var model = new jsong.Model();
 model.setCache({
    user: {
 ...
 // Note that the Sentinel object was returned rather than just the value "Jim Parsons."
 */
 ...
 * The getValueSync method differs from the asynchronous get methods (ex. get, getValues) in that it can be used to retrieve objects in addition to JSONG values.
 * @name getValueSync
 * @memberof Model.prototype
 * @function
...
/**
 * Set the {@link Model}'s local cache to a JSONG value. This method can be a useful way of mocking a remote document, or restoring state previously saved to local storage.
 * @name setCache
 * @memberof Model.prototype
 * @function
 * @arg {JSONG} jsong a JSONG object to use as the local cache.
 * @example
 // In this example we will retrieve data from the model and converted it into an HTML representation.
 // Creating a model and passing a data source that connects it to a remote JSONG model.
 var model = new jsong.Model(new XMLHttpDataSource('http://netflix.com/model.jsong'));
 ...
 // When this function is invoked all of the data should be available in the cache, except for those paths that caused an error to occur
 function() {
        var firstGenreList = this.bindSync(["genreList",0]);
        return "&lt;h1&gt;" + firstGenreList.getValueSync(["name"]) + "&lt;/h1&gt;\n" +
            "&lt;ul&gt;\n" +
            [0,1,2].
                // filter out indexes that have not been retrieved from the server yet
 ...
 * @memberof Model.prototype
 * @function
 * @arg {Path} boundPath the path referred to by the resulting Model. Typically the boundPath refers to another Path found in the JSONG model. If the boundPath does not evaluate to a JSONG Path but rather a branch node in the JSON Graph, at least one relative path to preload must be specified. Specifying a path to preload is necessary in these circumstances because the bound path must be evaluated to ensure it is fully optimized, but it is only possible to evaluate JSONG Paths that refer to a value. By preloading and evaluating a path along the bound path, it is possible to evaluate and fully optimize the bound path prefix.
 * @arg {...PathSet} relativePathsToPreload paths to values to preload before Model is created. These paths are relative to the bound path. If the boundPath does not refer to a path or a JSONG value, at least one path to a value must be preloaded. This is necessary because the a boundPath cannot be evaluated unless it refers to a JSONG value.
 ...
 * @example
 ...
 model.
 bind(["genreList", 0, 0], ["name"]).

 // retrieve the bound Model from the stream. When the Model arrives
 // it will be bound to the fully optimized path ["titles", 23] rather than the
 // requested path ["genreList", 0, 0].
 ...
 * @example
 ...
 function() {
        ...
        // Note that the bound path will be the optimized path ["genres", 123] rather
        // than the requested path ["genreList", 0].
        var firstGenreList = this.bindSync(["genreList",0]);
 ...
 * @example
 ...
 // {path:["friendsList", 0], value: ["friends", 929]}
 // and the ["name"] suffix will be added to value, producing
 // ["friends",929,"name"]
 [["name"]],
 // paths to retrieve after successful execution of function
 [["friendsList", "length"]],
 // the selector function is invoked with an array of the values in the results  returned from the functions. This array contains one result: ["friends", 123].
 function(results) {
                // bind a model to the location in the JSONG object returned by the function
...
/**
 * Sets the value at one or more places in the JSONG model. The set method accepts one or more {@link PathValue}s, each of which is a combination of a location in the document and the value to place there.  In addition to accepting  {@link PathValue}s, the set method also returns the values after the set operation is complete.
 * @name set
 * @memberof Model.prototype
 * @function
 * @arg {...PathValue} pathValue a location in the document and the value to set at that location.
 * @return {ModelResponse.<JSON>} an {@link Observable} stream containing the values in the JSONG model after the set was attempted.
 * @example
 //  Example: set values in various locations in the JSONG model
 ...
 // Create an {@link Observable} stream of {@link PathValue}s containing the values in the JSONG model after the set was attempted. Note that the code below does not immediately trigger the set operation, because {@link Observable}s are lazily-evaluated.  The set operation will be delayed until the next line of code, in which we attempt to retrieve the values from the {@link Observable} using its {@link Observable.prototype.forEach} method.
 var pathValues =
 model.
 set(
 {
     path: ["genreList", 0, 0, "rating"],
     value: 5
 },
 {
     path: ["genreList", 0, 1, "rating"],
     value: 4
 }).
 toPathValues();
 ...
 // The code above outputs the following {@link PathValue}s to the console in non-deterministic order.
 // {"path":["genreList",0,0,"rating"],"value":5}
 // {"path":["genreList",0,1,"rating"],"value":4}
 // Note that the values above were returned out of order. This was possible
 // because each PathValue provides sufficient context (ie the path) to
 // differentiate which value is being sent.
 */
...
/**
 * A Model object is used to execute commands against a {@link JSONG} object. {@link Model}s can work with a local JSONG object in its cache, or it can work with a remote {@link JSONG} objects through a {@link DataSource} object.
 * @constructor
 * @name Model
 * @arg {?DataSource} dataSource An object used to retrieve data from a remote model on the network.
